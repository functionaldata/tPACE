...
VCAM <- function(Lt,Ly,X,optnAdd=list(),optnVc=list()) {
  ...
  points(sort(X[,1]),phi1(sort(X[,1])),col=2,type='l',lty=2,lwd=2)
  plot(sort(X0[,2]),phiEst[order(X0[,2]),2],type='l',ylim=c(-1,3))
  points(sort(X[,2]),phi2(sort(X[,2])),col=2,type='l',lty=2,lwd=2)
  ...
  points(sort(stdT),beta0(sort(stdT)),col=2,type='l',lty=2,lwd=2)
  plot(T0,betaEst[,2],type='l')
  points(sort(stdT),beta1(sort(stdT)),col=2,type='l',lty=2,lwd=2)
  plot(T0,betaEst[,3],type='l')
  points(sort(stdT),beta2(sort(stdT)),col=2,type='l',lty=2,lwd=2)
