...
Eigen::MatrixXd RmullwlskUniversalDeriv( 
  const Eigen::Map<Eigen::VectorXd> & bw, 
  const std::string kernel_type, 
  const Eigen::Map<Eigen::MatrixXd> & tPairs, 
...
// Assumes the first row of tPairs is sorted in increasing order.
  ...
  const unsigned int nder = nder1 + nder2;
  const bool isSymmetric = (nder1 == nder2);
  
  if (npoly < 1) {
    Rcpp::stop("npoly should be not less than nder1 + nder2");
  }
  if (nder1 < 0 || nder1 > 1 || nder2 < 0 || nder2 > 1) {
    Rcpp::stop("nder should be either 0 or 1");
  }
  
  unsigned int nXcols = (npoly + 1) * (npoly + 2) / 2;
  ...
  for (unsigned int i = 0; i != xgridN; ++i) {  
    ...
    for (unsigned int j = (autoCov && isSymmetric)? i: 0; j != ygridN; ++j) { 
      ...
      if (meter >= nXcols && !bwCheck) { 
        ...
        Eigen::MatrixXd X(indxSize, nXcols);
        X.col(0).setOnes();
        unsigned int curCol = 1;
        for (unsigned int poly = 1; poly <= npoly; ++poly) {
          for (unsigned int d2 = 0; d2 <= poly; ++d2) {
            X.col(poly * (poly + 1) / 2 + d2) = 
              (lx.row(0).array() - xgrid(i)).pow(poly - d2) *
              (lx.row(1).array() - ygrid(j)).pow(d2);
            ++curCol;
          }
        }
        ...
        // Take the correct index and multiply by the factorial.
        // Rcpp::Rcout << npoly << ' ' << nder1 << ' ' << nder2 << '\n';
        mu(i,j)=beta(nder * (nder + 1) / 2 + nder2) * factorial(nder1) * 
                                                      factorial(nder2); 
      } else if(meter < nXcols) {
  ...
  if (autoCov && isSymmetric){
    return (
      Eigen::MatrixXd(mu.triangularView<Eigen::StrictlyUpper>().transpose()) +
      Eigen::MatrixXd(mu.triangularView<Eigen::Upper>())
    );
  } else {
    // ProfilerStop();
    return ( mu ); 
