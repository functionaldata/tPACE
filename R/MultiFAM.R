...
MultiFAM <- function(Y,X,ker='epan',nEval=51,alpha=0.5,supp=c(-2,2),optnsList=NULL){
  ...
  sbf <- SBFitting(Y,xiStdGrid,XiStd,h,supp=supp)
  
  xiGrid <- xiStdGrid%*%diag(sqrt(estLambda))
  
  bw <- h*sqrt(estLambda)
  
  sbf <- list(xi=xiGrid,mu=sbf$mY,SBFit=sbf$SBFit,bw=bw)
