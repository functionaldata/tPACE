...
getLogLik = function(ret, k, y = NULL, t = NULL){
  ...
  if(ret$optns$lean == FALSE){ # when input data is in ret
    y <- ret$inputData$y
    t <- ret$inputData$t
  }
  lambda = ret$lambda[1:k]
  sigma2 = ret$sigma2
  if(is.null(sigma2) && ret$optns$dataType == "Dense"){
    ymat = matrix(unlist(y),nrow=length(y), byrow=TRUE)
    sddiag = sqrt(diag(var(ymat)))
    sigma2 = sddiag*1e-4
    sigma2 = ConvertSupport(fromGrid = ret$obsGrid, toGrid = ret$workGrid, mu = sigma2)
  }
  logLik = 0
  phi = ret$phi[,1:k, drop=FALSE]

  if(ret$optns$dataType %in% c('Dense'
    ...
    for(i in 1:length(y)){ # TODO: imputation for DenseWithMV needed
      invtempSub = solve(Sigma_y, y[[i]] - ret$mu)
      logLik = logLik + log(detSigma_y) + invtempSub %*% (y[[i]] - ret$mu)
    }
    return(logLik)
  } else { # Sparse case
    if(is.null(sigma2)){ sigma2 <- ret$rho }
    if(ret$optns$error == TRUE && sigma2 <= ret$rho){
      # especially for the case when sigma2 is estimated to be <=0 and set to 1e-6
      sigma2 <- ret$rho
    }
    for(i in 1:length(y)){
      phi_i = ConvertSupport(fromGrid = ret$workGrid, toGrid = t[[i]],
        phi = phi)
      mu_i = ConvertSupport(fromGrid = ret$obsGrid, toGrid = t[[i]],
        mu = ret$mu)
      if(k == 1){
