...
FPCAregFunc <- function(depVar,  expVarScal = NULL, expVarFunc = NULL, regressionType = NULL, bwScalar = NULL, bwFunct = NULL){
  ...
  if( regressionType == 'Dense' ){
    ...
    if (!is.null(expVarFunc)){
      fittedPredCurvesList <- lapply( fpcaObjList, fitted)
    } else {
    ...
    for (i in 1:length(depVar$workGrid)) {
      ...
      for (j in 1:length(expVarFunc)){
        eval(parse(text=paste("Zvariables$funcPredictor", j, " = fittedPredCurvesList[[i]][,j]", sep='')))
      }
    ...
    # Deal with functional predictors
      ...
      for( i in 1:Q){
        for( j in i:Q){
          if(i == j){
            CCXX[ ((i-1)*L) + (1:L), ((j-1)*L) + (1:L) ] = fpcaObjList[[j]]$fittedCov
          } else {
            tempCCXX =  CrCovYX(Ly1 = fpcaObjList[[i]]$inputData$y, Ly2 = fpcaObjList[[j]]$inputData$y,
                               Lt1 = fpcaObjList[[i]]$inputData$t, Lt2 = fpcaObjList[[j]]$inputData$t,
                               Ymu1= fpcaObjList[[i]]$mu,  Ymu2= fpcaObjList[[j]]$mu);
            CCXX[ ((i-1)*L) + (1:L), ((j-1)*L) + (1:L) ] = tempCCX$smoothedCC;
            # And the transpose
            CCXX[ ((j-1)*L) + (1:L), ((i-1)*L) + (1:L) ] = t(tempCCX$smoothedCC);
          }
        }  
      }
      ...
      for (i in 1:Q){
        tempCCYX = CrCovYX(Ly1 = fpcaObjList[[i]]$inputData$y, Ly2 = depVar$inputData$y,
                                Lt1 = fpcaObjList[[i]]$inputData$t, Lt2 = depVar$inputData$t,
                                Ymu1= fpcaObjList[[i]]$mu,  Ymu2= depVar$mu);
      ...
      for (i in 1:Q){
        for (j in 1:ncol(expVarScal)){
          CCXZ[j+ (i-1)* ncol(expVarScal), ] = 
                     CrCovYZ (Ymu = fpcaObjList[[i]]$mu, Lt = fpcaObjList[[i]]$inputData$t, Ly = fpcaObjList[[i]]$inputData$y, 
                              Z = Zvariables[,j],support = fpcaObjList[[i]]$workGrid, bw = bwScalar)$smoothedCC
        }
      }
      ...
      for( i in 1:length(depVar$workGrid)) {
        ...
        covZX[1:(P-Q),  (P-Q+1):P] =   matrix( CCXZ[1:(P-Q), i])
        covZX[(P-Q+1):P,  1:(P-Q)] = t(matrix( CCXZ[1:(P-Q), i]))
        for( j0 in 1:Q){
          for( j1 in 1:Q){
            covZX[ P-Q+j0, P-Q+j1] = CCXX[ j0*L , j1*L]
          }
        }     
