...
FAM <- function(Y,Lx,Lt,K='epan',FVE=NULL,xi=NULL,h=NULL){
  ...
  tmpFPCA <- FPCA(Lx, Lt, optns = list(FVEthreshold=FVE))
    
  Xi <- tmpFPCA$xiEst
  d <- length(tmpFPCA$lambda)
  
  if (is.null(xi)==TRUE) {
    xi <- matrix(rep(seq(-2,2,length.out=N),d),nrow=N,ncol=d)
  }
  ...
  if (is.null(h)==TRUE) {
    ...
    for (j in 1:d) {
      options(warn = -1) 
      h[j] <- fdapace:::CVLwls1D(y=(Y-mean(Y)),t=Xi[,j],kernel='epan',npoly=1,nder=0,dataType='Dense',kFolds=20)/2
      options(warn = 0) 
    } 
  }
  
  fam <- matrix(nrow=nrow(xi),ncol=ncol(xi))
  for (j in 1:d) {
    fam[,j] <- fdapace:::Lwls1D(bw=h[j],kernel_type='epan',xin=sort(Xi[,j]),yin=(Y[order(Xi[,j])]-mean(Y)),xout=xi[,j],npoly=1,nder=0)
  }
  yMean <- mean(Y)
  
  fit <- list(mu=yMean, fam=fam, xi=xi)
  return(fit)
