...
#’
#’ @details \code{SBFitting} fits component functions of additive models for a scalar response and multivariate predictor based on the smooth backfitting algorithm proposed by Mammen et al. (1999) and intensively studied by Mammen and Park (2006), Yu et al. (2008), Lee et al. (2010, 2012) and so on. \code{SBFitting} only focuses on the local constant smooth backfitting estimator with multivariate predictor case. In fact, the case of univariate predictor is the same with the local constant kernel regression estimator (Nadaraya-Watson estimator) and local polynomial estimation can be extended similarly, so that those are omitted in the development. Support of the multivariate predictor is usually assumed to be a product of closed intervals. Especially in this development, one can designate an estimation support of additive models when the additive modeling is allowed over restricted intervals or one is interested in the additive modeling over the support (see Han et al., 2016). If one puts \code{X} on the argument of estimation points \code{x}, \code{SBFitting} returns estimated values of conditional mean responses given observed predictors.
#’
