...
test_that('Epanechnikov kernel matches', {
# degree 1
    expect_equal(lwls1d(0.3, 'epan', xin=x, yin=y, xout=c(0, 0.15, 0.35, 0.55, 3)), 
                 c(-0.075034417378891, 0.142526970448573, 0.380805485912617,   0.573996912896636, 0.232449699086771))
# degree 2
    expect_equal(lwls1d(0.3, 'epan', npoly=2, xin=x, yin=y, xout=c(0, 0.15, 0.35, 0.55, 3)), 
                 c(-0.125290762148466, 0.130962171169946, 0.442387233004433, 0.496519823276647, 0.346408319249383))
})


test_that('bandwidth works', {
    expect_equal(lwls1d(0.5, 'epan', xin=x, yin=y, xout=c(0, 0.15, 0.35, 0.55, 3)), 
                 c(-0.114913056431249,  0.132079727835864,   0.365292140767686, 0.569502194595946,   0.218632400355849))
    expect_equal(lwls1d(0.1, 'epan', xin=x, yin=y, xout=c(0, 0.15, 0.35, 0.55, 3)), 
...
test_that('Rectangular kernel matches', {
    expect_equal(lwls1d(0.3, 'rect', xin=x, yin=y, xout=c(0.15, 0.35, 0.55, 3)), 
                 c(0.140580458121389,   0.369243683204375,   0.611958566485952, 0.220769364459782))
# BUT THE BOUNDARY DOES NOT MATCH
})


test_that('Derivative works', {
    expect_equal(lwls1d(0.3, nder=1, xin=x, yin=y, xout=c(0, 0.15, 0.35, 0.55, 3)),
                        c(0.985197231385895,   1.767923028863129,   0.837595175365137, 0.929221260582535,  -0.689889054107760))
    expect_equal(lwls1d(0.3, npoly=2, nder=1, xin=x, yin=y, xout=c(0, 0.15, 0.35, 0.55, 3)),
