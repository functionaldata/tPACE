...
test_that('FPCAreg for scalar case returns correct results for a dense functional predictor',{
...
test_that('FPCAreg for scalar case returns correct results for a dense functional predictor and scalar predictors',{
...
test_that('FPCAreg for scalar case returns correct results for two dense functional predictors',{
...
test_that('FPCAreg for scalar case returns correct results for two dense functional predictors and two scalar ones',{
...
test_that('FPCAreg for scalar case returns correct results for a sparse functional predictor',{
  ...
  # Define the mean and 2 eigencomponents
  meanFunct <- function(s) s*0#+ 10*exp(-(s-5)^2)
  eigFunct1 <- function(s) +cos(2*s*pi/10) / sqrt(5)
  ...
  # Create beta_Func
  betaFunc = c(2,2) %*%  t(matrix(c(eigFunct1(s),eigFunct2(s)), ncol=2))
  x2 <- rt(N,2)
  ...
  CC_yx = AAA$smoothedCC;
  C_xx = diag(fpcaObjS$smoothedCov)
  
  par(mfrow=c(1,3))
  
  plot(s, CC_yx)
  Q = matrix( rep(a,100), ncol=100)
  W = cov(Q, yTrue)
  lines(s, diag(W), col='red' )

  sSmall = fpcaObjS$workGrid
  plot(sSmall, C_xx)
